...
export const PlayerProfileDialog = memo(function PlayerProfileDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-strong border-foreground/10 max-w-md sm:max-w-lg">
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-foreground/10 flex items-center justify-center">
              <span className="text-lg font-black text-foreground/80">{player.name.charAt(0).toUpperCase()}</span>
            </div>
            <div>
              <DialogTitle className="font-headline text-xl font-bold text-foreground/90">
                {player.name}
              </DialogTitle>
              <DialogDescription className="flex items-center gap-2 text-foreground/50">
        ...
        <div className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
            <Card className="bg-green-500/5 border-green-500/20 p-2 sm:p-3 text-center">
              ...
              <div className="text-[10px] text-foreground/40 font-medium">Team Wins</div>
            </Card>
            <Card className="bg-red-500/5 border-red-500/20 p-2 sm:p-3 text-center">
              ...
              <div className="text-[10px] text-foreground/40 font-medium">Team Losses</div>
            </Card>
            <Card className="bg-blue-500/5 border-blue-500/20 p-2 sm:p-3 text-center">
              ...
              <div className="text-[10px] text-foreground/40 font-medium">Matches</div>
            </Card>
          </div>
          ...
          {stats.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-xs font-semibold text-foreground/50 uppercase tracking-wider">Performance Stats</h4>
              <div className="space-y-2">
                {stats.map(([key, value]) => {
                  ...
                  return (
                    <div key={key} className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-bold text-foreground/90 tabular-nums">{numVal}</span>
                      </div>
              ...
              {totalStatValue > 0 && (
                <div className="flex items-center gap-2 pt-2 border-t border-foreground/5">
                  ...
                  <span className="text-xs text-foreground/40">Total contribution: <strong className="text-amber-400">{totalStatValue}</strong></span>
                </div>
              )}
            </div>
          )}
          ...
          {teamMatches.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-xs font-semibold text-foreground/50 uppercase tracking-wider">Recent Team Matches</h4>
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {teamMatches.slice(0, 5).map(m => {
                  ...
                  return (
                    <div key={m.id} className="flex items-center gap-2 p-2 rounded-lg bg-foreground/[0.02] border border-foreground/5">
                      <Badge className={`text-[9px] px-1.5 ${
                        won ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                        draw ? 'bg-foreground/5 text-foreground/40 border-foreground/10' :
