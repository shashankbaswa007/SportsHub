        ...
        >
            ...
            <motion.header variants={sectionVariants} className="relative">
                ...
                <div className="relative space-y-3 sm:space-y-4">
                    ...
                    <p className="text-foreground/60 text-sm sm:text-base lg:text-lg max-w-2xl">
                        Explore all competing teams and their rosters organized by sport
                    </p>

                    {/* Stats Summary */}
                    <div className="flex flex-wrap gap-2 sm:gap-4 pt-3 sm:pt-4">
                        <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-foreground/5 border border-foreground/10">
                            <Trophy className="h-3.5 sm:h-4 w-3.5 sm:w-4 text-yellow-400" />
                            <span className="text-xs sm:text-sm text-foreground/70">{sports.length} Sports</span>
                        </div>
                        <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-foreground/5 border border-foreground/10">
                            <Users className="h-3.5 sm:h-4 w-3.5 sm:w-4 text-blue-400" />
                            <span className="text-xs sm:text-sm text-foreground/70">{teams.length} Teams</span>
                        </div>
                        <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-foreground/5 border border-foreground/10">
                            <User className="h-3.5 sm:h-4 w-3.5 sm:w-4 text-green-400" />
                            <span className="text-xs sm:text-sm text-foreground/70">{players.length} Players</span>
                        </div>
                    </div>
                </div>
            </motion.header>
                            ...
                            >
                                ...
                                <div className={`flex items-center gap-2 sm:gap-4 pb-3 sm:pb-4 border-b ${accent.border.replace('border-', 'border-b-')}`}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-foreground/50 text-xs sm:text-sm">{sportTeams.length} team{sportTeams.length !== 1 ? 's' : ''} competing</p>
                                    </div>
                                    <button
                                        onClick={() => toggleFavoriteSport(sport)}
                                        className="p-2 rounded-lg hover:bg-foreground/10 transition-all shrink-0 group/fav"
                                        aria-label={isSportFavorite(sport) ? 'Remove from favorites' : 'Add to favorites'}
                                    >
                                        <Heart className={`h-5 w-5 transition-all ${isSportFavorite(sport) ? 'fill-red-500 text-red-500' : 'text-foreground/30 group-hover/fav:text-foreground/60'}`} />
                                    </button>
                                </div>
                                                ...
                                                `}>
                                                    ...
                                                    <CardHeader className="p-3 sm:p-4 pb-3 sm:pb-4">
                                                        <div className="flex items-start justify-between gap-2">
                                                            <CardTitle className="font-headline text-lg sm:text-xl lg:text-2xl font-bold text-foreground/95 group-hover:text-foreground transition-colors">
                                                                {team.name}
                                                            </CardTitle>
                                                            <div className="flex items-center gap-1.5 shrink-0">
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); toggleFavoriteTeam(team.id); }}
                                                                    className="p-1 rounded-md hover:bg-foreground/10 transition-all group/fav"
                                                                    aria-label={isTeamFavorite(team.id) ? 'Remove from favorites' : 'Add to favorites'}
                                                                >
                                                                    <Heart className={`h-4 w-4 transition-all ${isTeamFavorite(team.id) ? 'fill-red-500 text-red-500' : 'text-foreground/20 group-hover/fav:text-foreground/50'}`} />
                                                                </button>
                                                        ...
                                                        {teamPlayers.length > 0 ? (
                                                            <ul className="space-y-2 sm:space-y-3">
                                                                {teamPlayers.map(player => (
                                                                    <li 
                                                                        key={player.id} 
                                                                        className="flex items-center gap-2 sm:gap-3 p-2 sm:p-2.5 rounded-lg bg-foreground/5 hover:bg-foreground/10 hover:translate-x-1 transition-all group/player cursor-pointer"
                                                                        onClick={() => {
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <span className="font-medium text-foreground/90 group-hover/player:text-foreground transition-colors text-sm sm:text-base">
                                                                            {player.name}
                                                                        </span>
                                                                    </li>
                                                                ))}
                                                            ...
                                                            <div className="flex flex-col items-center justify-center py-6 sm:py-8 text-foreground/40">
